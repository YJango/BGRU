...
class M2MGRU(object):
    ...
    def __init__(self, learning_rate=1e-4, drop_out=0.3,Layers=[2,1,2], N_hidden=2048, N_cell=2048, D_input=39, D_out=120, Task_type='regression', L2_lambda=0.0, _EPSILON=1e-12):
        ...
        if Layers[0]!=0:
            #reshape for present GRU xy
            self.l_hid = lasagne.layers.ReshapeLayer(self.l_hid, (-1, present_steps*2, self.N_hidden/2))
            print('Reshape for BGRU',self.l_hid.output_shape) 

        #M2MGRU
        for i in range(Layers[1]):
            #present GRU 
            self.l_gru  =  lasagne.layers.recurrent.GRULayer(
                self.l_hid, self.N_cell/2, grad_clipping=10.,
                learn_init=1,resetgate=gate, updategate=gate, hidden_update=cell,backwards=False)
            #future GRU
            self.l_gru_b = lasagne.layers.recurrent.GRULayer(
                self.l_hid, self.N_cell/2, grad_clipping=10.,
                learn_init=1,resetgate=gate, updategate=gate, hidden_update=cell,backwards=True)
