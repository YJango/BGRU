...
def openx(f, mode=None, veclen=13):
    """openx an HTK format feature file for reading or writing.
...
class HTKFeat_write(object):
    ...
    def writevec(self, vec):
        ...
        if self.swap:
            ...
            numpy.array(vec, self.dtype).tofile(self.fh)
        self.filesize = self.filesize + self.nSamples
    def writeall(self, arr):
        for row in arr:
            self.writevec(row)
